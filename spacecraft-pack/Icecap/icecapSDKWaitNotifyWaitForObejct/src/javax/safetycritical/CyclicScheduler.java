...
final class CyclicScheduler extends Scheduler implements vm.Scheduler {
	...
	private static CyclicScheduler scheduler = null;
	...
	public Process getNextProcess() {
		...
		if (scjProcess.getTarget() instanceof MissionSequencer<?> && ((MissionSequencer<?>) (scjProcess.getTarget())).currState == State.END) {
			scjProcess.getTarget().cleanUp();
			CyclicScheduler.instance().stop(scjProcess.getProcess());
		}

		return scjProcess.getProcess();
	}
	...
	void start(MissionSequencer<?> seq) {
		...
		current = new ScjProcess(seq, new int[Const.HANDLER_STACK_SIZE_DEFAULT]);
		// devices.Console.println("CyclicScheduler.start:==>current:" + current
		// + "\n");
