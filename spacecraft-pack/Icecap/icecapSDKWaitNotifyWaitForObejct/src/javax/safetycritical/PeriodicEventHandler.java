...
/**
 * This class permits the automatic periodic execution of code. The <code>handleAsyncEvent</code> method behaves as if the handler were attached to a
 * periodic asynchronous event.
 * <p>
 * 
 ...
 *             An <code>IllegalArgumentException</code> is thrown if storage is null. But <code>ManagedEventHandler</code> does not require an
 *             <code>IllegalArgumentException</code> thrown if storage is null. It only specifies: a non-null maximum storage. This can be satisfied
 *             by implementing: if storage parameter is null, a default value is given. <br>
 */
@SCJAllowed
public abstract class PeriodicEventHandler extends ManagedEventHandler {
	 ...
	 * 
	 * @param priority
	 *            specifies the priority parameters for this periodic event handler. Must not be <code>null</code>.
	 * 
	 * @param release
	 *            specifies the periodic release parameters, in particular the start time, period and deadline miss handler. Note that a relative
	 *            start time is not relative to now, rather it is relative to the point in time when initialization is finished and the timers are
	 *            started. This argument must not be <code>null</code>.
	 * 
	 * @param storage
	 *            specifies the storage parameters for the periodic event handler. It must not be <code>null</code>.
	 * 
	 ...
	 * @
	 */
	public PeriodicEventHandler(PriorityParameters priority, PeriodicParameters release, StorageParameters storage, String name) {
	...
	public AbsoluteTime getNextReleaseTime(AbsoluteTime dest) {
		if (process.getState() == ScjProcess.State.SLEEPING)
			throw new IllegalStateException("the peh is not been started yet.");
		if (dest != null) {
	...
	public AbsoluteTime getNextReleaseTime() {
		if (process.getState() == ScjProcess.State.SLEEPING)
			throw new IllegalStateException("the peh is not been started yet.");
		AbsoluteTime dest = new AbsoluteTime(process.getNext());
		return dest;
	}
}
