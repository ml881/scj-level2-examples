...
public abstract class CyclicExecutive extends Mission
{
  ...
  {    
    final CyclicSchedule schedule =
      getSchedule ((PeriodicEventHandler[]) this.MSSetForMission.MSOs);
...
//    LocalRunnable privateLogic = new LocalRunnable(); 
    ...
    {  
      ...
      {     	  
//        privateLogic.handler = handlers[handlerIdx];
//        privateLogic.handler.privateMemory.enter(privateLogic);
  ...
  static private int find(PeriodicEventHandler[] handlerList, PeriodicEventHandler h) 
  {
	for (int i = handlerList.length-1; i >= 0; i--)
	  if (handlerList[i] == h) return i;
	return -1;
  }
  ...
  boolean feasible(PeriodicEventHandler[] handlerList, CyclicSchedule schedule)
  {
	  ...
	  for (int step = 0; step < frames.length; step++) {	
	      ...
	      for (int handlerIdx = 0; handlerIdx < n; handlerIdx++) {
	    	...
	    	if (next.compareTo(nextDeadline[h]) > 0) return false;
	    	nextDeadline[h].add(((PeriodicParameters)hx.getReleaseParameter()).getPeriod());
	      }
	      // advance to next frame
	      if (next.compareTo(startFrame.add(frames[step].duration)) > 0) return false;
	      next.add(frames[step].duration);
	  } 
	  // the schedule was OK for handlers in the frames
	  // check no handler left behind.
	  for (int h = 0; h < handlerList.length; h++)
		  if (nextDeadline[h].compareTo(next) <= 0) return false;
