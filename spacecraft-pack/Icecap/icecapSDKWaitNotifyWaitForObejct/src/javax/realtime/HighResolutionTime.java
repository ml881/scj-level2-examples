...
/**
 * <code>HighResolutionTime</code> is the base class for time given with nanosecond accuracy.
 * <p>
 * A time object in normalized form represents negative time if both components are nonzero and negative, or one is nonzero and negative and
 * the other is zero. <br>
 ...
 *             - semantic issue: method <code>set</code>: if parameter null, no Exception?
 *             <p>
 * 
 *             - implementation issue: <code>public int compareTo (Object arg0)</code> is inherited from <code>interface Comparable</code>
 *             <br>
 */
@SCJAllowed
public abstract class HighResolutionTime implements Comparable<HighResolutionTime> {
	...
	/*
	 * @ public invariant ( getMilliseconds() >= 0L && (0 <= getNanoseconds() && getNanoseconds() < 1000000) ) || ( getMilliseconds() <= 0L
	...
	/*
	 * @ behaviour requires true; ensures (millis-getMilliseconds())*NANOS_PER_MILLI + (nanos-getNanoseconds()) == 0;
	 * 
	 * @
	 */
	 ...
	 * 
	 * @param time
	 *            is the new value for <code>this</code>.
	 * 
	 * @throws ClassCastException
	 *             if the <code>time</code> parameter is not of the same class as <code>this</code>.
	 * @throws IllegalArgumentException
	 *             if the <code>time</code> parameter is not associated with the same clock as <code>this</code>, or when the
	 *             <code>time</code> parameter is null.
	 */
	/*
	 * @ public normal_behaviour requires time != null && this.getClass() == time.getClass();
	 * 
	 * ensures getMilliseconds() == time.getMilliseconds(); ensures getNanoseconds() == time.getNanoseconds(); ensures getClock() ==
	 * time.getClock(); also public exceptional_behaviour requires time == null; signals (IllegalArgumentException) true; also public
	 * exceptional_behaviour requires time != null && getClass() != time.getClass(); signals (ClassCastException) true;
	 * 
	 * @
	 */
	...
	/**
	 * Sets the millisecond component of <code>this</code> to the given argument, and the nanosecond component of <code>this</code> to 0.
	 * The clock is unchanged.
	 * 
	 * @param millis
	 *            is the new value of the millisecond component.
	 */
	/*
	 * @ public behaviour requires true; ensures getMilliseconds() == millis; ensures getNanoseconds() == 0;
	 * 
	 * @
	 */
	...
	/**
	 * Sets the millisecond and nanosecond components of <code>this</code> to the given arguments. The clock is unchanged.
	 * 
	 * @param millis
	 *            is the new value of the millisecond component.
	 * @param nanos
	 *            is the new value of the nanosecond component.
	 */
	/*
	 * @ public behaviour requires true; ensures getMilliseconds() - millis + (getNanoseconds() - nanos) / 1000000 == 0; ensures
	...
	/**
	 * @param time
	 * @return true just when the parameter <code>time</code> is of the same type and has the same values as <code>this</code>.
	 */
	/*
	 * @ public behaviour requires true;
	 * 
	 * ensures ( \result == ( time != null && getClass() == time.getClass() && getMilliseconds() == time.getMilliseconds() &&
	 * getNanoseconds() == time.getNanoseconds() && getClock() == time.getClock() ) );
	 * 
	 * @
	 */
	public boolean equals(HighResolutionTime time) {
		...
		return (this.getClass() == time.getClass()) && (this.millis == time.getMilliseconds()) && (this.nanos == time.getNanoseconds())
	 ...
	 * 
	 * @param time
	 *            is the second argument to the comparison.
	 * 
	 * @return a negative integer, zero, or a positive integer as <code>this</code> object is less than, equal to, or greater than
	 *         <code>time</code>.
	 * 
	 * @throws ClassCastException
	 *             if the <code>time</code> parameter is not of the same class as <code>this</code>.
	 ...
	 */
	/*
	 * @ public normal_behaviour requires time != null && this.getClass() == time.getClass() && this.getClock() == time.getClock();
	 * 
	 * ensures ( \result < 0 ==> ( (getMilliseconds() < time.getMilliseconds()) || (getMilliseconds() == time.getMilliseconds() &&
	 * getNanoseconds() < time.getNanoseconds()) ) ) || ( \result > 0 ==> ( (getMilliseconds() == time.getMilliseconds() && getNanoseconds()
	 * > time.getNanoseconds()) || (getMilliseconds() > time.getMilliseconds()) ) ) || ( \result == 0 ==> ( getMilliseconds() ==
	 * time.getMilliseconds() && getNanoseconds() == time.getNanoseconds() ) ); also public exceptional_behaviour requires time == null;
	 * signals (IllegalArgumentException) true; also public exceptional_behaviour requires time != null && getClass() != time.getClass();
	 * signals (ClassCastException) true; also public exceptional_behaviour requires time != null && getClass() == time.getClass() &&
	 * this.getClock() != time.getClock(); signals (IllegalArgumentException) true;
	 * 
	 * @
	 */
	...
	/* @ helper @ */final void setNormalized(final long ms, final int ns) {
		/*
		 * Examples: 3.12 millis = 3 millis and 120*1000 nanos 0.12 millis = 0 millis and 120*1000 nanos 0.00 millis = 0 millis and 0*1000
		 * nanos -3.12 millis = -3 millis and -120*1000 nanos -0.12 millis = 0 millis and -120*1000 nanos -0.00 millis = -0 millis and -
		 * 0*1000 nanos
		 */
