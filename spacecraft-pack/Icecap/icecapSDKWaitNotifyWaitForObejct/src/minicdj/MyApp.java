...
{ 
  ...
  final class MySequencer extends MissionSequencer 
  {
    ...
    MySequencer()
    {
      super (new PriorityParameters (Priorities.PR99), 
             new StorageParameters (Const.PRIVATE_MEM_SIZE, null, 0,0,0));
      
      devices.Console.println("---- MySequencer.constructor: super finished");
      
      mission = new Level0Safelet();    // with information about size of mission memory 
      mission.setup();
      
      devices.Console.println("MySequencer.constructor: MyMission created");
    }
    ...
    public Mission getNextMission() 
    {
      ...
      {
        devices.Console.println("\n ** MySequencer.getNextMission: null \n    missionTerminate: " + mission.terminationPending()); 
        mission.teardown();
      ...
      {
        devices.Console.println("\nMySequencer.getNextMission:" + mission + "\n    missionTerminate: " + mission.terminationPending()); 
        return mission;
      }
    } 
  }
