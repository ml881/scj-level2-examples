...
public class TestSCJPrioritySchedule2 {
	...
	private static class MyPeriodicEvh extends PeriodicEventHandler {
		...
		protected MyPeriodicEvh(PriorityParameters priority, PeriodicParameters periodic, 
				long memSize, int n, 
				AperiodicEventHandler aevh) {
			super(priority, periodic, new StorageParameters(memSize, new long[] { 256 }, 0,0,0));
			this.n = n;
			this.aevh = aevh;
		}
	...
	private static class MyAperiodicEvh extends AperiodicEventHandler {
		...
		public MyAperiodicEvh(PriorityParameters priority, AperiodicParameters release, long memSize, int n, 
				MissionSequencer<MyMission> missSeq) {
			super(priority, release, new StorageParameters(memSize, new long[] { 256 }, 0,0,0),null);
			this.n = n;
	...
	private static class MyMission extends Mission {
		...
		public void initialize() {
			...
			AperiodicEventHandler aevh = 
				new MyAperiodicEvh(new PriorityParameters(Priorities.PR98), 
					new AperiodicParameters(new RelativeTime(50, 0, Clock.getRealtimeClock()),
					null), 64, 1, missSeq);
			aevh.register();
